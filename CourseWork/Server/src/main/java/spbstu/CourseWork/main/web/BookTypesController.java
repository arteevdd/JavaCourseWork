...
public class BookTypesController {
    ...
    @GetMapping("/find-all")
    public ResponseEntity<List<BookTypes>> findAll(){
        try{
            List<BookTypes> bookTypesList = bookTypesService.findAll();
            return new ResponseEntity<>(bookTypesList, HttpStatus.OK);
        }catch (BookTypeNotFoundException e) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, e.getMessage());
        }
    ...
    public ResponseEntity<BookTypes> add(@RequestBody BookTypesDto bookTypeDTO){
        BookTypes bookType = bookTypesService.add(bookTypeDTO);
